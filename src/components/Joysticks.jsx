...
function parseFilename(f) {
  return { nombre: f.replace(/\.\w+$/, ''), img: `/images/joysticks/${f}` }
}
...
export default function Joysticks() {
  ...
  useEffect(() => {
    fetch('/api/joysticks').then(r => r.json())
  ...
  return (
    <section className="w-full py-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-white">Joysticks</h2>
          <p className="text-neutral-500 text-sm mt-1">Inalámbricos para PS4 y PS3</p>
        </div>
        {items.length === 0 ? (
