  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 30 }}
        ...
        className="relative w-full max-w-lg bg-[#111] rounded-2xl overflow-hidden shadow-2xl neon-border-blue" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-gradient-to-br from-blue-950/30 to-black p-10">
          <motion.img
            src={item.img} alt={item.nombre}
            className="w-full h-72 object-contain drop-shadow-[0_0_30px_rgba(59,130,246,0.3)]"
            initial={{ scale: 0.8 }} animate={{ scale: 1 }}
            transition={{ type: 'spring', damping: 20 }}
          />
        </div>
        <div className="p-6">
          <p className="font-bold text-white text-xl mb-5 neon-text-blue">{item.nombre}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10 text-center">
          ...
          <motion.p
            ...
            className="text-blue-400/70 text-sm mt-2"
          >
            Tiras LED, pen drives, cargadores y más
          </motion.p>
          <motion.div
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {items.map((item, i) => (
              <motion.div key={item.id}
                ...
                className="neon-card-blue neon-shimmer rounded-2xl overflow-hidden cursor-pointer group"
              >
                <div className="relative p-6 pb-4 bg-gradient-to-br from-blue-950/20 to-transparent">
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.08),transparent_70%)] group-hover:opacity-100 opacity-0 transition-opacity duration-500" />
                  <motion.img
                    src={item.img} alt={item.nombre}
                    className="relative z-10 w-full h-44 sm:h-52 object-contain drop-shadow-[0_0_20px_rgba(59,130,246,0.15)] group-hover:drop-shadow-[0_0_30px_rgba(59,130,246,0.35)] transition-all duration-500"
                  />
                </div>
                <div className="relative z-10 bg-black/40 px-5 py-4 border-t border-blue-500/10">
                  <p className="font-bold text-white text-base group-hover:neon-text-blue transition-all duration-300">{item.nombre}</p>
                  <p className="text-blue-400/50 text-xs mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">Click para ver más</p>
                </div>
              </motion.div>
            ))}
          </div>
